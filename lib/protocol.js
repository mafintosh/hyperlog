...
Protocol.prototype.resolvable = function (cb) {
  this._encode(4, empty, null, cb)
}

Protocol.prototype.sync = function (cb) {
...
Protocol.prototype._decode = function (data, cb) {
  switch (data[0]) {
    case 0: return this.emit('handshake', messages.Handshake.decode(data, 1), cb) || cb()
    case 1: return this.emit('have', messages.Log.decode(data, 1), cb) || cb()
    case 2: return this.emit('want', messages.Log.decode(data, 1), cb) || cb()
    case 3: return this.emit('node', messages.Node.decode(data, 1), cb) || cb()
    case 4: return this.emit('resolvable', cb) || cb()
    case 5: return this.emit('sync', cb) || cb()
  }
